...
app = FastAPI()
mcp_client = Client("http://mcp_server:9000/mcp")
...
async def chat(websocket: WebSocket):
    async with mcp_client:
        ...
        ollama_tools_list = []
        system_prompt = get_system_prompt(mcp_tools)
        primary_response_stream = [] # Includes initial prompt response and which will include any 'decisions' about having to call tools
        tool_calling_result_messages = [] # Includes messages with tool calling results that are going to serve as second stage prompt for secondary prompt response stream
        secondary_response_stream = [] # Includes secondary prompt response which includes tool calling results

        for tool in mcp_tools:
            # 'Translating' tools list to format that Ollama understands
            ollama_tools_list.append(
                {
                    "type": "function",
                    "function": {
                        "name": tool.name,
        ...
        #  Making sure that the sysem prompt is included on the server side
        messages.insert(0, {"role": "system", "content": system_prompt})

        # Handling the initial prompt response stream
        initial_stream = ollama.AsyncClient(
            host="https://ollama.com", headers={"Authorization": API_KEY}
        ).chat(
            model=SELECTED_MODEL,
            messages=messages,
            tools=ollama_tools_list,
            stream=True,
        )

        async for chunk in await initial_stream:

            # Handle tool calling if any is needed
            if chunk.message.tool_calls:
                result = "Couldn't get data from the tool call" # Default message in case result is not retrieved
                for call in chunk.message.tool_calls:
                    result = await mcp_client.call_tool(call.function.name, dict(call.function.arguments))

                    # Appending the result of tool call to be used in the second stage prompt
                    tool_calling_result_messages.append(
                        ollama.Message(
                            role="tool",
                            tool_name=call.function.name,
                            content=f"{call.function.name} tool calling result: {result}",
                        )
                    )

            # Send the initial prompt response stream through the WebSocket
            await websocket.send_text(chunk["message"]["content"])
            messages.append(chunk["message"])

        # Creating the secondary prompt response stream based on the previous tool calling results. This allows LLM to interpret the tool calling results.
        if tool_calling_result_messages:
            messages.append(
                ollama.Message(
                    role="system",
                    content="You were just given a result of a tool call. Use it appropriately",
                )
            )

            for message in tool_calling_result_messages:
                messages.append(message)

            secondary_response_stream = ollama.AsyncClient(
                host="https://ollama.com", headers={"Authorization": API_KEY}
            ).chat(
                model=SELECTED_MODEL, messages=messages, tools=ollama_tools_list, stream=True
            )
            async for chunk in await secondary_response_stream:
                await websocket.send_text(chunk["message"]["content"])

        await websocket.close()

def get_system_prompt(available_tools):
    SYSTEM_PROMPT = f"""You are a conversational AI assistant. You were created by Rafał Myśliwczyk as a portfolio project to promote Rafał Myśliwczyk as a job candidate.
        ...
        * Be framed as general impressions, not verified psychological facts.
        ...
        You have access to the following tools. They are available to you via standard MCP protocol. Use them only when necessary to answer questions. Always treat tool calls as structured actions, not guesses. After receiving the tool result, you may reason and answer naturally to the user. Adhere strictly to the following MCP schema when calling the tools. This includes using only the exact listed arguments.
